...
if (
  !all(
    c("ncdf4", "curl", "lubridate", "reshape2", "maps", "ggplot2") %in%
      installed.packages()
  )
) {
  install.packages(
    c("ncdfCF", "curl", "lubridate", "reshape2", "maps", "ggplot2"),
...
download_nc <- function(
  ...
  if (is.null(dl_time_step)) {
    ...
    dl_time_step %in%
      c("8d", "8D", "8day", "8Day", "8-day", "8-daily", "weekly")
  ) {
    dl_time_step <- "8-day"
  } else {
    stop(
  ...
  if (is.null(dl_bbox)) {
    ...
    stop(
      "Please provide a bounding box in the form of a vector with 4 values: c(lonmin, lonmax, latmin, latmax)"
    )
  } else if (any(dl_bbox < -180) | any(dl_bbox > 180)) {
    stop(
      "Please ensure that the longitude values in 'dl_bbox' are between -180 and 180."
    )
  } else if (any(dl_bbox < -90) | any(dl_bbox > 90)) {
    stop(
      "Please ensure that the latitude values in 'dl_bbox' are between -90 and 90."
    )
  ...
  if (dl_product == "ODATIS-MR") {
    ...
    if (is.null(dl_sensor)) {
      ...
      stop(
        "Please set 'dl_sensor' to either 'MODIS', 'MERIS', 'OLCI-A', or 'OLCI-B'"
      )
    }
    ...
    if (is.null(dl_correction)) {
      if (dl_sensor == "MODIS") {
        ...
        message(paste0(
          "No atmospheric correction chosen, defaulting to 'polymer' to match 'dl_sensor = ",
          dl_sensor,
          "'."
        ))
  ...
  if (dl_product == "SEXTANT" & dl_time_step != "day") {
    message(
      "SEXTANT data product only contains daily data. 'dl_time_step' adjusted accordingly."
    )
    dl_time_step <- "day"
  }
  ...
  while (current_date <= end_date) {
    ...
    url_year_doy <- paste0(
      substr(dl_date, start = 1, stop = 4),
      "/",
      strftime(as.Date(dl_date), format = "%j")
    )
    ...
    if (dl_product == "SEXTANT") {
      ...
      if (dl_time_step == "day") {
        ...
        url_8day_dates <- as.Date(paste(
          dl_year,
          dl_month,
          url_8day_days,
          sep = "/"
        ))
        if (length(url_8day_dates) < 3 | length(url_8day_dates) > 5) {
          stop("Something has gone wrong with the 8-day HTML scraping.")
        }
        ...
        dl_day <- substr(dl_date, start = 9, stop = 10)
        dl_date_flat <- paste0(
          format(dl_date, "%Y%m%d"),
          "-",
          format(dl_date + 7, "%Y%m%d")
        )
        ...
        if (year(dl_date + 7) > year(dl_date)) {
          dl_date_flat <- paste0(
            format(dl_date, "%Y%m%d"),
            "-",
            format(as.Date(paste(dl_year, 12, 31, sep = "/")), "%Y%m%d")
          )
        }
        ...
        dl_date_flat <- paste0(
          ...
          "-",
          format(ceiling_date(ymd(dl_date), "month") - days(1), "%Y%m%d")
        )
      } else {
    ...
    if (file.exists(file_name_full) & !overwrite) {
      message(paste0(
        file_name_full,
        " already exists. Set 'overwrite = TRUE' to force the download."
      ))
    } else {
      ...
      if (!is.null(dl_bbox) & dl_product == "ODATIS-MR") {
        ...
        tryCatch(
          {
            ...
            for (i in 1:length(names(nc_data$var))) {
              ...
              var_data_list[[paste0("var_data_", i)]] <- ncvar_get(
                nc = nc_data,
                varid = var_info_i$name,
                start = lonlat_start,
                count = lonlat_count
              )
              var_def_list[[paste0("var_def_", i)]] <- ncvar_def(
                name = var_info_i$name,
                units = var_info_i$units,
                dim = list(lon = lon_dim, lat = lat_dim),
                prec = var_info_i$prec,
                missval = var_info_i$missval,
            ...
            for (i in 1:length(var_def_list)) {
              ncvar_put(
                nc = output_nc_data,
                varid = var_def_list[[i]],
                vals = var_data_list[[i]]
              )
            }
            rm(i)
            ...
            ncatt_put(
              output_nc_data,
              varid = 0,
              attname = "northernmost_latitude",
              attval = max(lat_subset),
              definemode = TRUE
            )
            ncatt_put(
              output_nc_data,
              varid = 0,
              attname = "southernmost_latitude",
              attval = min(lat_subset),
              definemode = TRUE
            )
            ncatt_put(
              output_nc_data,
              varid = 0,
              attname = "easternmost_longitude",
              attval = max(lon_subset),
              definemode = TRUE
            )
            ncatt_put(
              output_nc_data,
              varid = 0,
              attname = "westernmost_longitude",
              attval = min(lon_subset),
              definemode = TRUE
            )
            ncatt_put(
              output_nc_data,
              varid = 0,
              attname = "spatial_subset",
              definemode = TRUE,
              attval = paste(
                "Spatially subset on",
                Sys.Date(),
            ...
            message(
              "Subsetting complete. The results are saved as : ",
              file_name_full
            )
          },
          error = function(e) {
        ...
        if (file.exists(file_name_full)) {
          if (grepl("bz2", file_name_full)) {
            result <- system(
              paste("bunzip2 -k -f", file_name_full),
              intern = TRUE,
              ignore.stderr = FALSE
            )
            if (length(result) != 0) {
...
plot_nc <- function(
  ...
  if (grepl("SPIM-ATL|CHL-ATL", nc_file)) {
    ...
    start_date <- as.Date(
      ncatt_get(nc_data, varid = 0)[["period_start_day"]],
      format = "%Y%m%d"
    )
    end_date <- as.Date(
      ncatt_get(nc_data, varid = 0)[["period_end_day"]],
      format = "%Y%m%d"
    )
  ...
  if (is.null(bbox)) {
    ...
    message(paste(
      "No bounding box provided, using full extent:",
      paste(round(bbox, 4), collapse = ", ")
    ))
  }

  # Filter df to bounding box
  var_df_sub <- var_df[var_df$lon >= bbox[1] & var_df$lon <= bbox[2], ]
  var_df_sub <- var_df_sub[
    var_df_sub$lat > bbox[3] & var_df_sub$lat <= bbox[4],
  ]
  ...
  plot_name <- file.path(
    output_dir,
    paste0(nc_var_name, "_", plot_date, ".png")
  )
